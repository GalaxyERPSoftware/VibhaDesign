...
const Pass = () => {
    ...
    //     try {
    //         const res = await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/getcartdetails`, {
    //             Customer_ID: user.UserID
    //         });

    //         if (res.data && res.data.AddToCartDetails) {
    //             const updatedCartData = res.data.AddToCartDetails.map(item => ({
    //                 ...item,
    //                 Product_Id: item.Product_Id || 2,
    //                 Category_Id: item.Category_Id || 0,         // ✅ Match with backend key
    //                 SubCategory_Id: item.SubCategory_Id || 0    // ✅ Match with backend key
    //             }));
    ...
    const handleDeleteItem = async (itemId) => {
        try {
            // await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/deletecartitem`, {
    ...
    const handleUpdateQuantity = async (itemId, newQuantity) => {
        try {
            ...
            //     await axios.post(`${process.env.REACT_APP_API_BASE_URL}/api/updatecartdetails`, {
            //         Product_ID: item.Product_Id,
            //         Customer_ID: user.UserID,
            //         Category_ID: item.Category_Id,
            //         SubCategory_ID: item.SubCategory_Id,
            ...
            // fetchCartData(); // Refresh cart after any change
